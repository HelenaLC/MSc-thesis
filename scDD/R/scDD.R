...
#'   performed (DZ).
#'   
#' @param sce An object of class \code{SingleCellExperiment} that contains 
...
#' \itemize{
#'   \item `gene`: gene name (matches rownames of sce)
...
scDD <- function(sce, 
  ...
  # check whether sce is a member of the SingleCellExperiment class
  if(!("SingleCellExperiment" %in% class(sce))){
    stop("Please provide a valid 'SingleCellExperiment' object.")
  }
  
  if (is.null(assayNames(sce)) || !("normcounts" %in% assayNames(sce))) {
  ...
  if (length(unique(colData(sce)[[condition]])) != 2 | 
      length(colData(sce)[[condition]]) != ncol(normcounts(sce))){
  ...
  tofit <- which(
           (rowSums(normcounts(sce)[,colData(sce)[[condition]]==ref]>0) >= 
             max(min.size,2,min.nonzero)) &
           (rowSums(normcounts(sce)[,colData(sce)[[condition]]!=ref]>0) >= 
              max(min.size,2,min.nonzero)))
  
  if (length(tofit) < nrow(normcounts(sce))){
    if(testZeroes){
      message(paste0("Notice: ", nrow(normcounts(sce))-length(tofit), 
  ...
  skipConstant <- which( 
                apply(normcounts(sce)[tofit,
                                       colData(sce)[[condition]]==ref], 1,
  ...
  oa <- c1 <- c2 <- vector("list", nrow(normcounts(sce)[tofit,]))
  bf <- den <- comps.all <- 
    comps.c1 <- comps.c2 <- rep(NA, nrow(normcounts(sce)[tofit,]))
  
  # cluster each gene in sce
  if (categorize)
  ...
  if (permutations == 0){
    ...
    genefit <- function(y){
      cond0 <- colData(sce)[[condition]][y>0]
      y <- log(y[y>0])
    ...
    if (categorize){
      out <- bplapply(1:nrow(normcounts(sce)[tofit,]), function(x) 
        genefit(normcounts(sce)[tofit[x],]))
    ...
    res_ks <- testKS(normcounts(sce)[tofit,], 
                     colData(sce)[[condition]], inclZero=FALSE)
    ...
    comps.c2  <- unlist(lapply(c2, function(x) luOutlier(x$class, min.size)))
      ...
      bf.perm <- vector("list", nrow(normcounts(sce)[tofit,]))
      names(bf.perm) <- rownames(normcounts(sce)[tofit,])
      
      if(parallelBy=="Permutations"){
        if(adjust.perms){
          C <- apply(normcounts(sce)[tofit,], 2, 
                     function(x) sum(x>0)/length(x))
          
          t1 <- proc.time()
          for (g in 1:nrow(normcounts(sce)[tofit,])){
            bf.perm[[g]] <- permMclustCov(normcounts(sce)[tofit[g],], 
                                          permutations, C, 
          ...
          t1 <- proc.time()
          for (g in 1:nrow(normcounts(sce)[tofit,])){
            bf.perm[[g]] <- permMclust(normcounts(sce[tofit[g],]), 
                                       permutations,
                                       colData(sce)[[condition]], 
        ...
        C <- apply(normcounts(sce)[tofit,], 2, function(x) sum(x>0)/length(x))
        bf.perm <- bplapply(1:nrow(normcounts(sce)[tofit,]), function(x) 
              permMclustGene(normcounts(sce)[tofit[x],], adjust.perms, 
                             permutations, colData(sce)[[condition]], 
      ...
      if (adjust.perms){
        pvals <- sapply(1:nrow(normcounts(sce)[tofit,]), function(x) 
  ...
  cats.all <- pvals.all <- rep(NA, nrow(normcounts(sce)))
  pvals.all[tofit] <- pvals
  
  if (categorize){
    message("Classifying significant genes into patterns")
    dd.cats <- classifyDD(normcounts(sce)[tofit,], colData(sce)[[condition]],
                        sig, oa, c1, c2, alpha=alpha, 
                        m0=m0, s0=s0, a0=a0, b0=b0, 
                        log.nonzero=TRUE, ref=ref, min.size=min.size)
  
    cats <- rep("NS", nrow(normcounts(sce)[tofit,]))
    cats[sig] <- dd.cats
  
    extraDP <- feDP(normcounts(sce)[tofit,], colData(sce)[[condition]], 
    ...
    NC.cats <- classifyDD(normcounts(sce)[tofit,], colData(sce)[[condition]],
  ...
  # zero test
  pvals.z <- rep(NA, nrow(normcounts(sce)))
  if (testZeroes){
    pvals.z <- testZeroes(normcounts(sce), colData(sce)[[condition]])
  ...
  # build MAP objects
  if (categorize){
    MAP1 <- matrix(1, nrow=nrow(normcounts(sce)), 
                 ncol=sum(colData(sce)[[condition]]==ref))
    MAP2 <- matrix(1, nrow=nrow(normcounts(sce)), 
                 ncol=sum(colData(sce)[[condition]]!=ref))
    MAP <- matrix(1, nrow=nrow(normcounts(sce)), 
                ncol=ncol(normcounts(sce)))
    rownames(MAP1) <- rownames(MAP2) <- rownames(MAP) <- rownames(sce)
    colnames(MAP1) <- colnames(sce[,colData(sce)[[condition]]==ref])
    colnames(MAP2) <- colnames(sce[,colData(sce)[[condition]]!=ref])
    colnames(MAP) <- colnames(sce)
    MAP1[normcounts(sce)[, colData(sce)[[condition]]==ref]==0] <- 0
    MAP2[normcounts(sce)[, colData(sce)[[condition]]!=ref]==0] <- 0
    MAP[normcounts(sce)==0] <- 0
  
    for (g in 1:nrow(normcounts(sce)[tofit,])){
      MAP1[tofit[g],][normcounts(sce[tofit[g], 
                  colData(sce)[[condition]]==ref])!=0] <- c1[[g]]$class 
      MAP2[tofit[g],][normcounts(sce[tofit[g], 
                  colData(sce)[[condition]]!=ref])!=0] <- c2[[g]]$class
      MAP[tofit[g],][normcounts(sce[tofit[g], ])!=0] <- oa[[g]]$class
    }
  }else{
  ...
  Genes = data.frame(gene=rownames(sce), 
                   DDcategory=cats.all, 
  ...
  metadata(sce)[["Genes"]] <- Genes
  metadata(sce)[["Zhat.combined"]] <- MAP
  metadata(sce)[["Zhat.c1"]] <- MAP1
  metadata(sce)[["Zhat.c2"]] <- MAP2
