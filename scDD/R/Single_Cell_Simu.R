...
if(Method %in%  c("DP", "DM")){
  MV[,1:2] <- t(sapply(1:length(samplename), function(x) 
    scDD:::calcMV(Dataset1[samplename[x],], FC=1, FC.thresh=FC2[x]^(-1/2), 
  ...
  MV[,1:2] <- t(sapply(1:length(samplename), function(x) 
    scDD:::calcMV(Dataset1[samplename[x],], FC=1, include.zeroes=FALSE)))
  MV[,3:4] <- t(sapply(1:length(samplename), function(x) 
    scDD:::calcMV(Dataset1[samplename[x],], FC=1, include.zeroes=TRUE)))
}

### Calculate parameter R and P in NB distribution
if(generateZero %in% c("empirical", "constant")){
  RP <- t(apply(MV[,1:2,drop=FALSE], 1, function(x) scDD:::calcRP(x[1], x[2])))
}else if (generateZero == "simulated"){
  RP <- t(apply(MV[,3:4,drop=FALSE], 1, function(x) scDD:::calcRP(x[1], x[2])))
}else{
...
if(Method=="DE") {
	desim <- scDD:::simuDE(Dataset1, Simulated_Data, DEIndex, samplename, 
	                Zeropercent_Base, f, FC, coeff, RP, modeFC,
	                generateZero, constantZero, varInflation)
	Simulated_Data <- desim[[1]]
  DE_FC <- desim[[2]]
} else if(Method=="DP"){
  dpsim <- scDD:::simuDP(Dataset1, Simulated_Data, DEIndex, samplename, 
  ...
  dmsim <- scDD:::simuDM(Dataset1, Simulated_Data, DEIndex, samplename, 
  ...
  DBsim <- scDD:::simuDB(Dataset1, Simulated_Data, DEIndex, samplename, 
                  Zeropercent_Base, f, FC2, coeff, RP, modeFC, DP,
                  generateZero, constantZero, varInflation)
